...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    *result_count = r;
    ...
    if (n == 1) {
        char** res = alloc_grid();
        for (int i = 0; i < r; i++) {
            memcpy(res[i], grid[i], c);
        }
        return res;
    }

    if (n % 2 == 0) {
        char** full = alloc_grid();
        for (int i = 0; i < r; i++) {
    ...
    char** detonate_from(char** S) {
        char** next = alloc_grid();
        for (int i = 0; i < r; i++) {
    ...
    char** initial_clone = alloc_grid();
    for (int i = 0; i < r; i++) memcpy(initial_clone[i], grid[i], c);

    char** grid1 = detonate_from(initial_clone);
    if (n % 4 == 3) {
        for (int i = 0; i < r; i++) free(initial_clone[i]);
        free(initial_clone);
        return grid1;
    }

    char** grid2 = detonate_from(grid1);
    for (int i = 0; i < r; i++) free(initial_clone[i]);
    free(initial_clone);
    for (int i = 0; i < r; i++) free(grid1[i]);
    free(grid1);
