...
int hackerlandRadioTransmitters(int x_count, int* x, int k) {
    ...
    while (i < x_count) {
        count++;
        // This is the leftmost house to cover
        int loc = x[i];
        // Move right to the furthest house within k of loc
        while (i < x_count && x[i] <= loc + k) {
            i++;
        }
        // Place transmitter at the last house in this range
        int transmitter = x[i - 1];
        // Skip all houses covered by transmitter
        while (i < x_count && x[i] <= transmitter + k) {
            i++;
        }
    }
